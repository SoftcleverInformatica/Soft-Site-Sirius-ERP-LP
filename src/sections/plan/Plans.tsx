	...
	return (
		<section className='flex w-full flex-col items-center' id='planos'>
			<div className={`flex w-full flex-col gap-6 px-4 pt-40 md:px-10 md:pt-20 lg:max-w-5xl lg:gap-20 lg:px-24 lg:pt-28 xl:max-w-6xl 2xl:max-w-[90rem]`}>
				...
				<div className='flex flex-col justify-center gap-5 rounded-[40px] border border-primary/50 px-2 py-10 md:grid md:grid-cols-2 md:grid-rows-2 xl:flex xl:flex-row'>
					{planos.map((_, i) => {
						return (
							<div key={i} className={`flex flex-col gap-1 rounded-[32px] p-4 pt-4 ${_.recommended ? 'hover:bg-amber-500' : ''} `}>
								<span className='h-6 pl-3 font-bold text-black/85'>{_.recommended ? 'Recomendado' : ''}</span>
								<div className={`flex w-[23rem] flex-1 flex-col justify-between gap-4 rounded-3xl bg-white px-4 py-8 transition delay-150 ease-in-out lg:gap-5 lg:px-8 lg:py-12`}>
									<div>
									...
									<div className='flex flex-col gap-1 lg:gap-4'>
										...
										<ButtonExpert color='blue' label='Fale com um especialista' />
									</div>
								</div>
							</div>
						);
